                    ...
                    if biaoJi == "下一页":
                        ...
                        if page > 3:
                            for perpage in range(2, 4):
                                ...
                                yield Request(url=requestUrl, meta=response.meta, headers=self.headers,
                                              cookies=self.cookie,
        ...
        try:
            ...
            if (isinstance(url, str)):
                if (url.startswith('http://') or url.startswith('https://')):
                    ...
                    picture = requests.get(url=url, headers=self.headers)
                    ...
                    for x in imgs:
                        if (x.startswith('http://') or x.startswith('https://')):
                            path = pa + 'picture_{0}_{1}_{2}.png'.format(fileName,
                                                                         time.strftime("%Y%m%d", time.localtime()),
                                                                         str(int((time.time()) * 1000)))
                            picture = requests.get(url=x, headers=self.Picture_headers)
                            with open(path, 'wb')as f:
                                f.write(picture.content)
                            geshihua = path[3:].replace('\\', '/')
                            savePath = TYSavePath + geshihua
