...
#     name ='21cnjy_math_middle_data'
#     def __init__(self):
#         self.B = 10
#         self.difficult_indexs = {'1': "容易", '2': "较易", '3': "普通", '4': "较难", '5': "困难"}
#         ua = UserAgent()
#         user_agent = ua.random
#         self.cookie ={'chid': '1772b5b2c5311c4295510e4f1a17ebbe1d182aa90b0285df5d0bab8139be663ba%3A2%3A%7Bi%3A0%3Bs%3A4%3A%22chid%22%3Bi%3A1%3Bi%3A3%3B%7D',
...
#                       '_csrf': 'cfd8248cfd75a0493903988b3d91bf8ee2c121bfc2449a1ed3c0a1b48008aa16a%3A2%3A%7Bi%3A0%3Bs%3A5%3A%22_csrf%22%3Bi%3A1%3Bs%3A32%3A%22rE1Sb7IP98RNcaQOg0dcJKkfmP7nU3Si%22%3B%7D',
#                       'Hm_lvt_5d70f3704df08b4bfedf4a7c4fb415ef': '1516005797',
#                       'Hm_lpvt_5d70f3704df08b4bfedf4a7c4fb415ef': '1516005810'}
#
#         self.heaeder = {
...
#                                                 perXueQiParamDict['question_channel_type'] = perTiXing
#                                                 perXueQiParamDict['question_type_index'] = tixingDict[perTiXing]
#                                                 yield Request(url=start_url, meta=perXueQiParamDict,cookies=self.cookie, headers=self.heaeder,callback=self.param_item)
#                                         elif(perTiXing == '多选题'):
#                                             if (perXueQiParamDict['jieduan'] == '小学资源导航'):
#                                                 # for page in range(1, 3):  # 爬去的页
#                                                 timeStamp = int(time.time()* 1000)
...
#                         timeStamp = int(time.time()* 1000)
#                         perurl = 'https://zujuan.21cnjy.com/question/list?categories={0}&question_channel_type={1}&difficult_index=&exam_type=&kid_num=&grade_id%5B%5D=0&grade_id%5B%5D=7&grade_id%5B%5D=8&grade_id%5B%5D=9&sortField=time&filterquestion=0&page={2}&page={2}&_={3}'.format(
#                             response.meta['zhangjieNumber'], response.meta['question_type_index'], str(perPages),str(timeStamp))
#                         yield Request(url=perurl, meta=response.meta, cookies=self.cookie, headers=self.heaeder,
#                                       callback=self.param_question)
#
#                 elif (pages < 3 and pages > 1):
#                     totlePages =pages+1
#                     for perPages in range(1, totlePages):
#                         timeStamp = int(time.time()* 1000)
#                         perurl = 'https://zujuan.21cnjy.com/question/list?categories={0}&question_channel_type={1}&difficult_index=&exam_type=&kid_num=&grade_id%5B%5D=0&grade_id%5B%5D=7&grade_id%5B%5D=8&grade_id%5B%5D=9&sortField=time&filterquestion=0&page={2}&page={2}&_={3}'.format(
#                             response.meta['zhangjieNumber'], response.meta['question_type_index'], str(perPages),str(timeStamp))
#                         yield Request(url=perurl, meta=response.meta, cookies=self.cookie, headers=self.heaeder,
