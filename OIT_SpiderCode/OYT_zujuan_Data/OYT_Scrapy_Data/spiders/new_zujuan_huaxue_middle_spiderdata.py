...
#     name = 'zujuan_huaxue_middle_data'
#     custom_settings = {
...
#         self.filename = 'zujuan_huaxue_middle_data'
...
#         }
#         path = 'd:\\OITData\\zujuan\\{0}\\zujuan_huaxue_middle_param\\'.format(time.strftime('%Y%m%d',time.localtime(time.time())))
#
#     #测试cookie值是否有效用
...
#
#     def start_requests(self):
#         with open('d:\\xiti10001\\zujuan\\{0}\\zujuan_huaxue_middle_param\\categories_huaxue_{0}.txt'.format(time.strftime('%Y%m%d',time.localtime(time.time())))) as f:
#             needStr = f.read()
...
#                             start_url = 'http://www.zujuan.com/question/list?categories={0}&question_channel_type={1}&difficult_index=&exam_type=&kid_num=&grade_id%5B%5D=0&grade_id%5B%5D=7&grade_id%5B%5D=8&grade_id%5B%5D=9&sortField=time&page={2}&_={3}'.format(numberNumber, i, '1',str(timeStamp))
#                             # print(start_url)
#                             if i == 1:
...
#                     timeStamp = int(time.time()) * 1000
#                     start_url = 'http://www.zujuan.com/question/list?categories={0}&question_channel_type={1}&difficult_index=&exam_type=&kid_num=&grade_id%5B%5D=0&grade_id%5B%5D=7&grade_id%5B%5D=8&grade_id%5B%5D=9&sortField=time&page={2}&_={3}'.format(
#                         response.meta['zhangjieNumber'], response.meta['question_channel_index'], str(perPages),
#                         str(timeStamp))
#                     yield Request(url=start_url, meta=response.meta, cookies=self.new_cookieVule, headers=self.hearders,
#                                   callback=self.param_question)
#
