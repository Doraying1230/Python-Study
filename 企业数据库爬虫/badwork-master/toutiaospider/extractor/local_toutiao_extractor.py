    ...
    def list_extractor(self, html, keyword):
        ...
        try:
            ...
            for each in content['data']:
                if each.keys()[0] != 'cell_type':
                    title = each.get('title')
                    article_url = each.get('article_url')
                    # if each.get('has_image'):
                    #     img_list = list(map(lambda x: x.get('url'), each.get('image_detail')))
                    #     for img_url in img_list:
                    #         img_ali_dict = self.deal_imgs(img_url)
                    if article_url:
                        list_results.append(
                            {
                                'title':title,
